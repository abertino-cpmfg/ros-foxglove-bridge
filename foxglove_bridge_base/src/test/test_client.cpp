...
std::future<FetchAssetResponse> waitForFetchAssetResponse(std::shared_ptr<ClientInterface> client) {
  ...
  client->setBinaryMessageHandler(
    [promise = std::move(promise)](const uint8_t* data, size_t dataLength) mutable {
      ...
      offset += 1;
      const size_t mediaTypeOrMsgLength = static_cast<size_t>(ReadUint32LE(data + offset));
      offset += 4;
      response.mediaTypeOrMsg =
        std::string(reinterpret_cast<const char*>(data + offset), mediaTypeOrMsgLength);
      offset += mediaTypeOrMsgLength;
